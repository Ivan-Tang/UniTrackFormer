...
def train_kfold(model_class, loss_fn_class, dataset_class, data_dir, detectors, event_ids, num_folds=5, num_epochs=10, device='cpu'):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(all_event_ids)):
        ...
        # 创建数据集和数据加载器
        train_dataset = dataset_class(data_dir=data_dir, detectors_df=detectors, event_ids=train_ids)
        val_dataset = dataset_class(data_dir=data_dir, detectors_df=detectors, event_ids=val_ids)
        ...
        # 训练模型
        train(model, loss_fn, train_loader, val_loader, optimizer, num_epochs=num_epochs, device=device)
